...
function safeStorageGet(key) {
  try {
    ...
    console.warn(`localStorage get blocked for "${key}"`, error);
    return null;
  }
}
...
function safeStorageSet(key, value) {
  try {
    ...
    console.warn(`localStorage set blocked for "${key}"`, error);
    return false;
  }
}
...
function safeStorageRemove(key) {
  try {
    ...
    console.warn(`localStorage remove blocked for "${key}"`, error);
  }
}
...
function loadLanguagePreference() {
  const stored = safeStorageGet('pageLanguage');
...
function saveFavorites() {
  safeStorageSet('favoriteMOS', JSON.stringify(favoriteMOS));
}
...
async function initPortal() {
  ...
  document.getElementById('apply-modal-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      // Clear form
      e.target.reset();
      safeStorageRemove(APPLY_FORM_DRAFT_KEY);
